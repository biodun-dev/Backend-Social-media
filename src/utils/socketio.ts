...
const SECRET_KEY = process.env.JWT_SECRET!; // Ensure this matches your JWT signing key
...
export const initSocket = (httpServer: HttpServer) => {
  ...
  io.on('connection', async (socket: Socket) => {
    ...
    // Update the socketId for the connected user
    try {
      const user = await User.findByIdAndUpdate(
        userId,
        { socketId: socket.id },
        { new: true, useFindAndModify: false }
      ) as (IUser & { _id: any }) | null; // Type assertion
      if (user) {
